...
lazy_static! {
    ...
    static ref RE_MOVE_FILE: Regex = Regex::new(r"^\*{3}\s*Move\s+File:\s*(.+?)\s*->\s*(.+)$").unwrap();
    ...
    while i < end_idx {
        ...
        if let Some(caps) = RE_UPDATE_FILE.captures(line) {
            ...
            current_op = Some(PatchOperation {
                ...
                file_path: caps.get(1).map(|m| m.as_str().trim().to_string()).unwrap_or_default(),
                new_path: None,
                hunks: Vec::new(),
                content: None,
            });
...
pub fn parse_v4a_patch(patch_content: &str) -> PyResult<String> {
    ...
    serde_json::to_string(&operations).map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))
}
...
*** End Patch"#;
        ...
        assert_eq!(ops[0].hunks[0].context_hint, Some("context hint".to_string()));
