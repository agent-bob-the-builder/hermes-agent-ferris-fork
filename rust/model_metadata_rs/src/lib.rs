...
const MAX_COMPLETION_KEYS: &[&str] = &["max_completion_tokens", "max_output_tokens", "max_tokens"];
...
fn search_nested(value: &serde_json::Value, keys: &[&str]) -> Option<i64> {
    ...
    if let serde_json::Value::Array(arr) = value {
        for item in arr {
            if let Some(f) = search_nested(item, keys) {
                return Some(f);
            }
        }
    }
    if let serde_json::Value::Object(map) = value {
        for v in map.values() {
            if let Some(f) = search_nested(v, keys) {
                return Some(f);
            }
        }
    }
...
pub fn extract_context_length(json_payload: &str) -> Option<i64> {
    let Ok(value) = serde_json::from_str::<serde_json::Value>(json_payload) else {
        return None;
    };
...
pub fn extract_pricing(json_payload: &str) -> String {
    ...
    fn search(
        v: &serde_json::Value,
        pricing: &mut serde_json::Map<String, serde_json::Value>,
    ) -> bool {
...
pub fn model_id_matches(candidate_id: &str, lookup_model: &str) -> bool {
    ...
    if let Some((_, bare)) = candidate_id.rsplit_once('/') {
        return bare == lookup_model;
    }
    false
}
