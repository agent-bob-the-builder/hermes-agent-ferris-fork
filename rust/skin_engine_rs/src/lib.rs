...
impl SkinConfig {
    ...
    #[getter]
    fn branding_dict(&self) -> HashMap<String, String> {
        self.branding.clone()
    }
    #[getter]
    fn tool_prefix(&self) -> String {
        self.tool_prefix.clone()
    }
    #[getter]
    fn tool_emojis(&self) -> HashMap<String, String> {
        self.tool_emojis.clone()
    }
    #[getter]
    fn banner_logo(&self) -> String {
        self.banner_logo.clone()
    }
    #[getter]
    fn banner_hero(&self) -> String {
        self.banner_hero.clone()
    }
    ...
    fn get_spinner_list(&self, key: &str) -> Vec<String> {
        self.spinner
            .get(key)
            .and_then(|v: &YamlValue| v.as_sequence())
            .map(|seq: &serde_yaml::Sequence| {
                seq.iter()
                    .filter_map(|item: &YamlValue| item.as_str().map(String::from))
                    .collect()
            })
            .unwrap_or_default()
    }
...
fn vd() -> HashMap<String, YamlValue> {
    HashMap::new()
}
...
macro_rules! skin_data {
    ($name:expr, $desc:expr, $colors:expr, $spinner:expr, $branding:expr, $tp:expr, $logo:expr, $hero:expr) => {{
        ...
        d.insert(
            "spinner".to_string(),
            serde_yaml::to_value(&$spinner).unwrap(),
        );
        d.insert(
            "branding".to_string(),
            serde_yaml::to_value(&$branding).unwrap(),
        );
        d.insert("tool_prefix".to_string(), sv($tp));
        if !$logo.is_empty() {
            d.insert("banner_logo".to_string(), sv($logo));
        }
        if !$hero.is_empty() {
            d.insert("banner_hero".to_string(), sv($hero));
        }
        d
    }};
...
fn make_builtin_skins() -> HashMap<&'static str, SkinData> {
    ...
    // --- default ---
    {
        ...
        c.insert("banner_border", "#CD7F32");
        c.insert("banner_title", "#FFD700");
        c.insert("banner_accent", "#FFBF00");
        c.insert("banner_dim", "#B8860B");
        c.insert("banner_text", "#FFF8DC");
        c.insert("ui_accent", "#FFBF00");
        c.insert("ui_label", "#4dd0e1");
        c.insert("ui_ok", "#4caf50");
        c.insert("ui_error", "#ef5350");
        c.insert("ui_warn", "#ffa726");
        c.insert("prompt", "#FFF8DC");
        c.insert("input_rule", "#CD7F32");
        c.insert("response_border", "#FFD700");
        c.insert("session_label", "#DAA520");
        c.insert("session_border", "#8B8682");
        let mut b = HashMap::new();
        b.insert("agent_name", "Hermes Agent");
        b.insert(
            "welcome",
            "Welcome to Hermes Agent! Type your message or /help for commands.",
        );
        b.insert("goodbye", "Goodbye! ⚕");
        b.insert("response_label", " ⚕ Hermes ");
        b.insert("prompt_symbol", "❯ ");
        b.insert("help_header", "(^_^)? Available Commands");
        skins.insert(
            "default",
            skin_data!(
                "default",
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#9F1C1C");
        c.insert("banner_title", "#C7A96B");
        c.insert("banner_accent", "#DD4A3A");
        c.insert("banner_dim", "#6B1717");
        c.insert("banner_text", "#F1E6CF");
        c.insert("ui_accent", "#DD4A3A");
        c.insert("ui_label", "#C7A96B");
        c.insert("ui_ok", "#4caf50");
        c.insert("ui_error", "#ef5350");
        c.insert("ui_warn", "#ffa726");
        c.insert("prompt", "#F1E6CF");
        c.insert("input_rule", "#9F1C1C");
        c.insert("response_border", "#C7A96B");
        c.insert("session_label", "#C7A96B");
        ...
        let mut b = HashMap::new();
        b.insert("agent_name", "Ares Agent");
        b.insert(
            "welcome",
            "Welcome to Ares Agent! Type your message or /help for commands.",
        );
        b.insert("goodbye", "Farewell, warrior! ⚔");
        b.insert("response_label", " ⚔ Ares ");
        b.insert("prompt_symbol", "⚔ ❯ ");
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#555555");
        c.insert("banner_title", "#e6edf3");
        c.insert("banner_accent", "#aaaaaa");
        c.insert("banner_dim", "#444444");
        c.insert("banner_text", "#c9d1d9");
        c.insert("ui_accent", "#aaaaaa");
        c.insert("ui_label", "#888888");
        c.insert("ui_ok", "#888888");
        c.insert("ui_error", "#cccccc");
        c.insert("ui_warn", "#999999");
        c.insert("prompt", "#c9d1d9");
        c.insert("input_rule", "#444444");
        c.insert("response_border", "#aaaaaa");
        c.insert("session_label", "#888888");
        c.insert("session_border", "#555555");
        skins.insert(
            "mono",
            skin_data!("mono", "Monochrome — clean grayscale", c, vd(), vd(), "┊"),
        );
    }
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#4169e1");
        c.insert("banner_title", "#7eb8f6");
        c.insert("banner_accent", "#8EA8FF");
        c.insert("banner_dim", "#4b5563");
        c.insert("banner_text", "#c9d1d9");
        c.insert("ui_accent", "#7eb8f6");
        c.insert("ui_label", "#8EA8FF");
        c.insert("ui_ok", "#63D0A6");
        c.insert("ui_error", "#F7A072");
        c.insert("ui_warn", "#e6a855");
        c.insert("prompt", "#c9d1d9");
        c.insert("input_rule", "#4169e1");
        c.insert("response_border", "#7eb8f6");
        c.insert("session_label", "#7eb8f6");
        c.insert("session_border", "#4b5563");
        skins.insert(
            "slate",
            skin_data!("slate", "Cool blue — developer-focused", c, vd(), vd(), "┊"),
        );
    }
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#2A6FB9");
        c.insert("banner_title", "#A9DFFF");
        c.insert("banner_accent", "#5DB8F5");
        c.insert("banner_dim", "#153C73");
        c.insert("banner_text", "#EAF7FF");
        c.insert("ui_accent", "#5DB8F5");
        c.insert("ui_label", "#A9DFFF");
        c.insert("ui_ok", "#4caf50");
        c.insert("ui_error", "#ef5350");
        c.insert("ui_warn", "#ffa726");
        c.insert("prompt", "#EAF7FF");
        c.insert("input_rule", "#2A6FB9");
        c.insert("response_border", "#5DB8F5");
        c.insert("session_label", "#A9DFFF");
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#B7B7B7");
        c.insert("banner_title", "#F5F5F5");
        c.insert("banner_accent", "#E7E7E7");
        c.insert("banner_dim", "#4A4A4A");
        c.insert("banner_text", "#D3D3D3");
        c.insert("ui_accent", "#E7E7E7");
        c.insert("ui_label", "#D3D3D3");
        c.insert("ui_ok", "#919191");
        c.insert("ui_error", "#E7E7E7");
        c.insert("ui_warn", "#B7B7B7");
        c.insert("prompt", "#F5F5F5");
        c.insert("input_rule", "#656565");
        c.insert("response_border", "#B7B7B7");
        c.insert("session_label", "#919191");
    ...
    {
        let mut c = HashMap::new();
        c.insert("banner_border", "#C75B1D");
        c.insert("banner_title", "#FFD39A");
        c.insert("banner_accent", "#F29C38");
        c.insert("banner_dim", "#7A3511");
        c.insert("banner_text", "#FFF0D4");
        c.insert("ui_accent", "#F29C38");
        c.insert("ui_label", "#FFD39A");
        c.insert("ui_ok", "#4caf50");
        c.insert("ui_error", "#ef5350");
        c.insert("ui_warn", "#ffa726");
        c.insert("prompt", "#FFF0D4");
        c.insert("input_rule", "#C75B1D");
        c.insert("response_border", "#F29C38");
        c.insert("session_label", "#FFD39A");
...
fn build_skin_config(data: &SkinData) -> SkinConfig {
    ...
    let default_colors: HashMap<String, String> = default_data
        .get("colors")
        .and_then(|v: &YamlValue| serde_yaml::from_value(v.clone()).ok())
        .unwrap_or_default();
    let mut colors: HashMap<String, String> = default_colors;
    if let Some(new_colors) = data.get("colors") {
        if let Ok(c) = serde_yaml::from_value::<HashMap<String, String>>(new_colors.clone()) {
            colors.extend(c);
        }
    }

    let default_spinner: HashMap<String, YamlValue> = default_data
        .get("spinner")
        .and_then(|v: &YamlValue| serde_yaml::from_value(v.clone()).ok())
        .unwrap_or_default();
    let mut spinner: HashMap<String, YamlValue> = default_spinner;
    if let Some(new_spinner) = data.get("spinner") {
        if let Ok(s) = serde_yaml::from_value::<HashMap<String, YamlValue>>(new_spinner.clone()) {
            spinner.extend(s);
        }
    }

    let default_branding: HashMap<String, String> = default_data
        .get("branding")
        .and_then(|v: &YamlValue| serde_yaml::from_value(v.clone()).ok())
        .unwrap_or_default();
    let mut branding: HashMap<String, String> = default_branding;
    if let Some(new_branding) = data.get("branding") {
        if let Ok(b) = serde_yaml::from_value::<HashMap<String, String>>(new_branding.clone()) {
            branding.extend(b);
        }
    }

    let default_tool_prefix = default_data
        .get("tool_prefix")
        .and_then(|v: &YamlValue| v.as_str())
        .unwrap_or("┊");
    let tool_prefix = data
        .get("tool_prefix")
        .and_then(|v: &YamlValue| v.as_str())
        .unwrap_or(default_tool_prefix);

    let tool_emojis: HashMap<String, String> = data
        .get("tool_emojis")
        .and_then(|v: &YamlValue| serde_yaml::from_value(v.clone()).ok())
        .unwrap_or_default();
    ...
    let description = data
        ...
        .and_then(|v: &YamlValue| v.as_str())
        .unwrap_or("");
    let banner_logo = data
        .get("banner_logo")
        .and_then(|v: &YamlValue| v.as_str())
        .unwrap_or("");
    let banner_hero = data
        .get("banner_hero")
        .and_then(|v: &YamlValue| v.as_str())
        .unwrap_or("");
    ...
    SkinConfig {
        ...
        tool_emojis,
        banner_logo: banner_logo.to_string(),
        banner_hero: banner_hero.to_string(),
    }
}
...
fn do_load_skin(name: &str) -> SkinConfig {
    ...
    if user_file.is_file() {
        if let Some(data) = load_skin_data_from_yaml(&user_file) {
            return build_skin_config(&data);
        }
    }
    if let Some(data) = BUILTIN_SKINS.get(name) {
        return build_skin_config(data);
    }
...
fn list_skins() -> Vec<HashMap<String, String>> {
    ...
    if skins_path.is_dir() {
        if let Ok(entries) = std::fs::read_dir(&skins_path) {
            let mut user_skins: Vec<_> = entries
                .filter_map(|e| e.ok())
            ...
            for entry in user_skins {
                if let Some(data) = load_skin_data_from_yaml(&entry.path()) {
                    ...
                    if result.iter().any(|r| r.get("name") == Some(&skin_name)) {
                        continue;
                    }
                    let description = data
                        .get("description")
...
#[pyfunction]
fn load_skin(name: String) -> SkinConfig {
    do_load_skin(&name)
}
#[pyfunction]
fn get_active_skin() -> SkinConfig {
    let name = ACTIVE_SKIN_NAME.read().unwrap().clone();
    do_load_skin(&name)
}
#[pyfunction]
fn set_active_skin(name: String) -> SkinConfig {
    let mut n = ACTIVE_SKIN_NAME.write().unwrap();
    *n = name.clone();
    do_load_skin(&name)
}
#[pyfunction]
fn get_active_skin_name() -> String {
    ACTIVE_SKIN_NAME.read().unwrap().clone()
}
...
fn init_skin_from_config(_py: Python<'_>, config: &Bound<'_, PyDict>) -> PyResult<()> {
    ...
    if !name.is_empty() {
        ...
        set_active_skin("default".to_string());
    }
    Ok(())
}

#[pyfunction]
fn get_active_prompt_symbol() -> String {
    let name = ACTIVE_SKIN_NAME.read().unwrap().clone();
    do_load_skin(&name)._get_branding_key("prompt_symbol", "❯ ")
}
#[pyfunction]
fn get_active_help_header() -> String {
    let name = ACTIVE_SKIN_NAME.read().unwrap().clone();
    do_load_skin(&name)._get_branding_key("help_header", "(^_^)? Available Commands")
}
#[pyfunction]
fn get_active_goodbye() -> String {
    let name = ACTIVE_SKIN_NAME.read().unwrap().clone();
    do_load_skin(&name)._get_branding_key("goodbye", "Goodbye! ⚕")
}
...
fn get_prompt_toolkit_style_overrides() -> HashMap<String, String> {
    ...
    overrides.insert(
        "completion-menu".to_string(),
        format!("bg:#1a1a2e {}", text),
    );
    overrides.insert(
        "completion-menu.completion".to_string(),
        format!("bg:#1a1a2e {}", text),
    );
    overrides.insert(
        "completion-menu.completion.current".to_string(),
        format!("bg:#333355 {}", title),
    );
    overrides.insert(
        "completion-menu.meta.completion".to_string(),
        format!("bg:#1a1a2e {}", dim),
    );
    overrides.insert(
        "completion-menu.meta.completion.current".to_string(),
        format!("bg:#333355 {}", label),
    );
    ...
    overrides.insert(
        "clarify-active-other".to_string(),
        format!("{} italic", title),
    );
