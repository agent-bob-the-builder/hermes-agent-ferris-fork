...
def _try_load_rust_pb():
    ...
    try:
        ...
        # Prepend venv site-packages so _prompt_builder_rs is found before
...
class AIAgent:
    ...
    def _rust_api_call_fn(self, api_messages_json: str) -> str:
                ...
                resp_dict["choices"] = [
                    {
                        ...
                        "message": {
                            "role": getattr(first.message, "role", None)
                            if hasattr(first.message, "message")
                            else None,
                            ...
                            if hasattr(first.message, "tool_calls")
                            and first.message.tool_calls
                            else [],
                        },
                    }
                ]
        ...
        # Handle OpenAI SDK ChatCompletion objects (model_dump provides proper JSON-serializable dict)
        if hasattr(response, "model_dump"):
            return _json.dumps(response.model_dump())
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        offloaded to the Rust prompt_builder_rs extension for ~10x faster
        ...
        if (
            ...
            if mod is not None:
                try:
                    ...
                    logger.debug(
                        "Rust prompt builder returned empty/error (%s), "
        ...
        if getattr(self, "_deferred_routing", False):
            ...
            if _routed_client is not None:
                ...
                if (
                    ...
                    self._client_kwargs["default_headers"] = dict(
        ...
        try:
            ...
            if _try_load_tool_dispatch_rs() is not None:
                try:
                    ...
                    logger.debug(
                        "Rust tool dispatch failed (%s), falling back to Python", rs_err
                    )
            ...
            for i, (tc, name, args) in enumerate(parsed_calls):
                ...
                if r is None:
                    results[i] = (
                        name,
                        args,
                        f"Error: tool '{name}' result not returned from Rust",
                        0.0,
                        True,
                    )
                else:
    ...
    def run_conversation(
        ...
        if _rust_result is not None:
            ...
            _rust_result["response_previewed"] = getattr(
        ...
        while (
            ...
            if not self.quiet_mode:
                ...
                self._vprint(
                    f"{self.log_prefix}   📊 Request size: {len(api_messages)} messages, ~{approx_tokens:,} tokens"
                )
                self._vprint(
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        if self._empty_content_retries < 3:
                            ...
                            # Use a friendly fallback message so the user isn't left hanging
                            final_response = "抱歉，我没有生成有效的回复。请尝试换一种方式表达，我会更好地帮助你。"
                            empty_msg = {
                                "role": "assistant",
                                "content": final_response,
                                "reasoning": reasoning_text,
